...
/*
  * Transaction fees parser. Java version of components/brave_wallet_ui/common/hooks/transaction-parser.ts.
  */
public class ParsedTransactionFees {
    ...
    protected ParsedTransactionFees(String gasLimit, String gasPrice,
            String maxPriorityFeePerGas, String maxFeePerGas, double gasFee,
            double gasFeeFiat, boolean isEIP1559Transaction, String missingGasLimitError,
            double gasPremium, double gasFeeCap) {
    ...
    public static ParsedTransactionFees parseTransactionFees(TransactionInfo txInfo, NetworkInfo selectedNetwork,
            Double networkSpotPrice, BigInteger solFeeEstimatesFee) {
        TxDataUnion txDataUnion = txInfo.txDataUnion;
        TxData1559 txData = txDataUnion.getEthTxData1559();
        FilTxData filTxData = txDataUnion.getFilTxData();
        ...
        final String gasLimit = isFilTransaction ? filTxData.gasLimit : (txData != null ? txData.baseData.gasLimit : "");
        ...
        final boolean isEIP1559Transaction = !maxPriorityFeePerGas.isEmpty() && !maxFeePerGas.isEmpty();
//         final String gasFeeFormat = "%.8f";
        final double gasFee = isSolTransaction ? solFeeEstimatesFee != null
            ? Utils.fromHexWei(solFeeEstimatesFee.toString(), networkDecimals) : 0.0d
            : Utils.fromHexWei(isEIP1559Transaction
                ? Utils.multiplyHexBN(maxFeePerGas, gasLimit)
                : Utils.multiplyHexBN(gasPrice, gasLimit), networkDecimals);

//         final String gasFeeFiatFormat = "%.2f";
        final double gasFeeFiat = gasFee * networkSpotPrice;
        final String missingGasLimitError = isSolTransaction ? null : checkForMissingGasLimitError(gasLimit);
        final double gasPremium = isFilTransaction ?
                Utils.fromHexWei(filTxData.gasPremium, networkDecimals) : 0.0d;
        final double gasFeeCap = isFilTransaction ?
                Utils.fromHexWei(filTxData.gasFeeCap, networkDecimals) : 0.0d;

        ParsedTransactionFees parsedTransactionFees = new ParsedTransactionFees(gasLimit, gasPrice,
            maxPriorityFeePerGas, maxFeePerGas, gasFee, gasFeeFiat, isEIP1559Transaction, missingGasLimitError,
            gasPremium, gasFeeCap);
