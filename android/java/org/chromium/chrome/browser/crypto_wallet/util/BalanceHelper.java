...
public class BalanceHelper {
    ...
    public static void getNativeAssetsBalances(JsonRpcService jsonRpcService,
        ...
        for (AccountInfo accountInfo : accountInfos) {
            ...
            if (selectedNetwork.coin == CoinType.SOL) {
                AsyncUtils.GetSolanaBalanceResponseContext context = addBalanceResponseContext(
                    contexts, new AsyncUtils.GetSolanaBalanceResponseContext(
                        balancesMultiResponse.singleResponseComplete),
                    accountInfo.address, null);
                jsonRpcService.getSolanaBalance(accountInfo.address, selectedNetwork.chainId, context);
            } else if (accountInfo.coin == CoinType.FIL) {
                // TODO: FIL placeholder
            } else {
                AsyncUtils.GetBalanceResponseContext context = addBalanceResponseContext(
                    contexts, new AsyncUtils.GetBalanceResponseContext(
                        balancesMultiResponse.singleResponseComplete),
                    accountInfo.address, null);
                jsonRpcService.getBalance(accountInfo.address, accountInfo.coin, selectedNetwork.chainId, context);
            }
        }
    ...
    * Only collect balances for current network. Return a nested map that is grouped by accounts first and then tokens.
    */
    public static void getBlockchainTokensBalances(JsonRpcService jsonRpcService,
        ...
        if (jsonRpcService == null) return;
        HashMap<String, HashMap<String, Double>> blockchainTokensBalances = new HashMap<String, HashMap<String, Double>>();
            ...
            for (BlockchainToken token: tokens) {
                if (accountInfo.coin == CoinType.ETH) {
                    if (selectedNetwork.chainId.equals(token.chainId)) {
                        if (token.isErc721) {
                            AsyncUtils.GetErc721TokenBalanceResponseContext context = addBalanceResponseContext(
                                contexts, new AsyncUtils.GetErc721TokenBalanceResponseContext(
                                    balancesMultiResponse.singleResponseComplete),
                                accountInfo.address, token);
                            jsonRpcService.getErc721TokenBalance(
                                token.contractAddress, token.tokenId != null ? token.tokenId : "",
                                accountInfo.address, token.chainId, context);
                        } else {
                            AsyncUtils.GetErc20TokenBalanceResponseContext context = addBalanceResponseContext(
                                contexts, new AsyncUtils.GetErc20TokenBalanceResponseContext(
                                    balancesMultiResponse.singleResponseComplete),
                                accountInfo.address, token);
                            jsonRpcService.getErc20TokenBalance(
                                token.contractAddress, accountInfo.address, token.chainId, context);
                        }
                    }
                } else if (accountInfo.coin == CoinType.SOL) {
                    if (selectedNetwork.chainId.equals(token.chainId)) {
                        AsyncUtils.GetSplTokenAccountBalanceResponseContext context = addBalanceResponseContext(
                            contexts, new AsyncUtils.GetSplTokenAccountBalanceResponseContext(
                                balancesMultiResponse.singleResponseComplete), 
                            accountInfo.address, token);
                        jsonRpcService.getSplTokenAccountBalance(accountInfo.address, token.contractAddress, token.chainId, context);
                    }
                } else {
                    // TODO: FIL placeholder
                }
        ...
        balancesMultiResponse.setWhenAllCompletedAction(() -> {
            for (AsyncUtils.GetBalanceResponseBaseContext context : contexts) {
                ...
                Double tokenBalance = (context.error == ProviderError.SUCCESS && context.balance != null && !context.balance.isEmpty())
                        ? Utils.fromHexWei(context.balance, decimals) : 0.0d;
                if (blockchainTokensBalances.containsKey(context.accountAddress)) {
                    blockchainTokensBalances.get(context.accountAddress).put(tokenKey, tokenBalance);
                } else {
                    HashMap<String, Double> perAccountBlockchainTokensBalances = new HashMap<String, Double>();
                    perAccountBlockchainTokensBalances.put(tokenKey, tokenBalance);
                    blockchainTokensBalances.put(context.accountAddress, perAccountBlockchainTokensBalances);
                }
    ...
    private static <T extends AsyncUtils.GetBalanceResponseBaseContext> T addBalanceResponseContext(
            ArrayList<AsyncUtils.GetBalanceResponseBaseContext> contexts,
            T context, String accountAddress, BlockchainToken token) {
