...
class RewardsPanelCoordinatorTest : public BrowserWithTestWindowTest {
 public:
  void SetUp() override {
    ...
    auto observer =
        MakePanelObserver([this](const mojom::RewardsPanelArgs& args) {
          last_args_ = args.Clone();
        });

    coordinator_ = RewardsPanelCoordinator::GetOrCreateForBrowser(browser());
    DCHECK(coordinator_);
    coordinator_->AddObserver(observer.get());
