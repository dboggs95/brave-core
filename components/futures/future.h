...
struct PromiseExecutor<void(base::OnceCallback<void(Args...)>)> {
...
template <typename T>
struct PromiseExecutor<void(base::OnceCallback<void(T)>)> {
...
template <typename T>
struct PromiseExecutor<void(Promise<T>)> {
  using Promise = Promise<T>;
...
struct PromiseExecutor<void (C::*)(Args...)>
    : public PromiseExecutor<void(Args...)> {};

template <typename C, typename... Args>
struct PromiseExecutor<void (C::*)(Args...) const>
    : public PromiseExecutor<void(Args...)> {};

template <typename... Args>
struct PromiseExecutor<void (*)(Args...)>
    : public PromiseExecutor<void(Args...)> {};

template <typename F>
struct PromiseExecutor
    : public PromiseExecutor<decltype(&std::decay_t<F>::operator())> {};

template <typename F>
auto MakeFuture(F&& f) {
  typename PromiseExecutor<F>::Promise promise;
  auto future = promise.GetFuture();
  PromiseExecutor<F>::Execute(std::forward<F>(f), std::move(promise));
  return future;
}
