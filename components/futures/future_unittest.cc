...
TEST_F(FutureTest, MakeFuture1) {
  ...
  MakeFuture([&](base::OnceCallback<void(int)> resolve) {
...
TEST_F(FutureTest, MakeFuture0) {
  ...
  MakeFuture([&](base::OnceCallback<void()> resolve) {
...
TEST_F(FutureTest, MakeFutureN) {
  std::tuple<int, bool> values = {0, false};

  MakeFuture([&](base::OnceCallback<void(int, bool)> resolve) {
    std::move(resolve).Run(42, true);
  }).AndThen(base::BindLambdaForTesting([&](std::tuple<int, bool> v) {
    values = v;
  }));
